<script lang="ts">
  ...
  $effect.pre(() => {
    let logScale = settings.app.general.logScale;
    let documentStyle = document.documentElement.style;

    if (logScale === "1") {
      documentStyle.setProperty("font-size", "medium");
    } else if (logScale === "2") {
      documentStyle.setProperty("font-size", "large");
    } else if (logScale === "3") {
      documentStyle.setProperty("font-size", "x-large");
    } else if (logScale === "0") {
      documentStyle.setProperty("font-size", "small");
    }
  });
</script>
