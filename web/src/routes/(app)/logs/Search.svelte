<script lang="ts">
  ...
  const difficultyMap = $appContext.difficultyMap;
  const encounterMap = $appContext.encounterMap;
  const bossList = $appContext.guardianRaidBosses;
  const classList = $appContext.classes;
