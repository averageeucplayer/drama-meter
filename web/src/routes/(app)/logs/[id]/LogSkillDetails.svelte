<script lang="ts">
  ...
  let supportBuffs = $derived.by(() => {
    ...
    return cast.hits.reduce(
      (acc, hit) => {
        const buffs = getSkillCastSupportBuffs($appContext.classesMap, $appContext.supportClassIds, hit, encounterDamageStats);
        acc.buff += buffs.buff;
  ...
  let allGroupedBuffs = $derived.by(() => {
    if (!cast) return;
    return cast.hits.map((hit) => getSkillCastBuffs($appContext.classesMap, $appContext.supportClassIds, hit, encounterDamageStats, player, buffType));
  });
