<script lang="ts">
  ...
  let buffChartSeries = $derived(
    getSupportSynergiesOverTime($appContext.classesMap, $appContext.supportClassIds, enc, encounter.fightStart, encounter.lastCombatPacket, 5000)
  );
  ...
  $effect(() => {
    if (
      ...
      if (chartType === ChartType.AVERAGE_DPS) {
        ...
        chartOptions = getAverageDpsChart($appContext.bossHpMap, chartablePlayers, legendNames, chartPlayers, bossChart, deathTimes);
        ...
        chartOptions = getRollingDpsChart($appContext.bossHpMap, chartablePlayers, legendNames, chartPlayers, bossChart, deathTimes);
      } else if (chartType === ChartType.SKILL_LOG && player && player.entityType === EntityType.PLAYER) {
        if (hasSkillDetails) {
          chartOptions = getDetailedSkillLogChart(
            $appContext.classesMap,
            $appContext.supportClassIds,
            player,
        ...
        chartOptions = getSupportSynergiesOverTimeChart($appContext.bossHpMap,buffChartLegend, buffChartSeries, "_1_", buffChartBosses);
      } else if (chartType === ChartType.AP_BUFF) {
        chartOptions = getSupportSynergiesOverTimeChart($appContext.bossHpMap,buffChartLegend, buffChartSeries, "_0_", buffChartBosses);
      } else if (chartType === ChartType.IDENTITY) {
        chartOptions = getSupportSynergiesOverTimeChart($appContext.bossHpMap,buffChartLegend, buffChartSeries, "_2_", buffChartBosses);
      } else if (chartType === ChartType.HYPER_BUFF) {
        chartOptions = getSupportSynergiesOverTimeChart($appContext.bossHpMap,buffChartLegend, buffChartSeries, "_3_", buffChartBosses);
      }
    }
  });
