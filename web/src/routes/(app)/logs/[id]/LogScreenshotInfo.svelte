<script lang="ts">
  ...
  let raidGate = $derived($appContext.raidGates[encounter.currentBossName]);

  let bossHpBars = $derived.by(() => {
    let boss = encounter.entities[encounter.currentBossName];
    if (boss) {
      return Math.ceil((boss.currentHp / boss.maxHp) * getBossHpBars($appContext.bossHpMap, boss.name, boss.maxHp));
    }
    return undefined;
  });
</script>
