<script lang="ts">
  ...
  const bossHpBarColors = ["#D16F23", "#9F3930", "#582469", "#2B3A63", "#246977", "#798816", "#E7B826"];
  ...
  let bossTotalBars = $derived.by(() => {
    if (Object.hasOwn($appContext.bossHpMap, boss.name) && settings.app.meter.bossHpBar) {
      return getBossHpBars($appContext.bossHpMap, boss.name, boss.maxHp);
    } else {
      return 1;
    }
  });
