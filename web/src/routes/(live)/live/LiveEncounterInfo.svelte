<script lang="ts">
  ...
  let durationPretty = $derived.by(() => {
	...
	const onAction: MouseEventHandler<HTMLElement> = async (event) => {
  ...
  <div data-tauri-drag-region class="flex w-full items-center justify-between">
    ...
    <div data-tauri-drag-region class="flex cursor-default items-center gap-1">
      {#if settings.app.general.bossOnlyDamage}
        <QuickTooltip tooltip="Boss Only Damage">
          <div class="boss scale-35 shrink-0" data-tauri-drag-region></div>
          <!-- <img src="/images/icons/boss.png" alt="Boss Only Damage" class="size-5 shrink-0"  /> -->
        </QuickTooltip>
      {/if}
    ...
    {#if !screenshot.state}
      ...
      <div class="flex items-center gap-0.5 text-neutral-300">
        ...
        <!-- minimize window -->
        <button data-action="hide" class="group" onclick={onAction}>
