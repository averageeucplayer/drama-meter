import { BossHpLog, type DamageStats, type Entity, type Esther, type IdentityLogType, type IdentityLogTypeValue } from "$lib/types";
...
export const preventDefault: MouseEventHandler<Window> = (event) => event.preventDefault();
...
export async function checkForUpdate() {
  try {
    ...
    const isAvailable = update?.available;

    if (isAvailable) {
      updateInfo.available = true;
      updateInfo.manifest = { body: update.body };
    } else {
...
export function getBossHpBars(bossHpMap: Record<string, number>, bossName: string, bossMaxHp: number) {
...
export function getEstherFromNpcId(estherMap: Esther[], npcId: number): string {
  for (const esther of estherMap) {
    if (esther.npcIds.includes(npcId)) return esther.name;
  }
