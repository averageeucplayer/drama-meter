  ...
  return {
    ...
    tooltip: {
      ...
      formatter(params: any[]) {
        ...
        params.forEach((param) => generateTooltip(bossHpMap, param, bossTooltips, totalDps, tree, deathTimes, time, length));
        ...
        tooltip: {
          formatter: function (param: any) {
            ...
            if (param.value[2].hits.length > 0) {
              output += skillCastBreakdownTooltip(classesMap, supportClassIds, player, param.value[2], encounterDamageStats);
            }
            output += "</div>";
            output += "</div>";
            return output;
...
function skillCastBreakdownTooltip(
  ...
  for (const [i, hit] of skillCast.hits.entries()) {
    const groupedBuffs = getSkillCastBuffs(classesMap, supportClassIds, hit, encounterDamageStats, player);
    const supportBuffs = getSkillCastSupportBuffs(classesMap, supportClassIds, hit, encounterDamageStats);
