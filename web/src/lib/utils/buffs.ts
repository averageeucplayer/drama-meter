...
export function filterStatusEffects(
  ...
  if (isPartySynergy(buff)) {
    ...
    if (!skillCastLogFilter || skillCastLogFilter === "Party") {
      if (isSupportBuff(supportClassIds, buff)) {
        key = makeSupportBuffKey(classesMap, buff);
      } else {
...
export function isSupportBuff(supportClasses: number[], statusEffect: StatusEffect) {
...
export function makeSupportBuffKey(classesMap: Record<number, string>, statusEffect: StatusEffect) {
  ...
  for (const type of buffTypes) {
    for (const buffId of (hit as any)[type.key1]) {
      ...
      if (buffs.hasOwnProperty(buffId)) {
        filterStatusEffects(classesMap, supportClassIds, groupedBuffs, buffs[buffId], buffId, player, undefined, undefined, buffType);
      }
    }
  }
...
export function getSkillCastSupportBuffs(
  classesMap: Record<number, string>,
  supportClassIds: number[],
  hit: SkillHit,
  encounterDamageStats: EncounterDamageStats) {
  ...
  for (const type of buffTypes) {
    for (const buffId of (hit as any)[type.key1]) {
      ...
      if (buffs.hasOwnProperty(buffId)) {
        ...
        getHitSupportBuffs(classesMap, supportClassIds, hit.damage, buffs[buffId], supportBuffs);
      }
    }
  }
  return supportBuffs;
...
function getHitSupportBuffs(
  classesMap: Record<number, string>,
  supportClassIds: number[],
  hitDamage: number,
  buff: StatusEffect,
  supportBuffs: SkillChartSupportDamage) {
  if (isPartySynergy(buff) && isSupportBuff(supportClassIds, buff)) {
    const key = makeSupportBuffKey(classesMap, buff);
