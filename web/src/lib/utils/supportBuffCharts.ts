...
function addStatusEffect(
  ...
  supportClasses: number[],
  map: Map<string, Map<number, StatusEffect>>, effect: [string, StatusEffect]) {
  const [id, statusEffect] = effect;
  if (
    !isPartySynergy(statusEffect) ||
    !isSupportBuff(supportClasses, statusEffect) ||
    statusEffect.source.name === bardSerenadeOfAmplification
  ) {
    return;
  }
  const key = makeSupportBuffKey(classesMap, statusEffect);
...
export function getSupportSynergiesOverTime(
  ...
  Object.entries(state.encounter!.encounterDamageStats.buffs).forEach((effect) =>
    addStatusEffect(classesMap, supportClasses, groupedSupportSynergies, effect)
  );
