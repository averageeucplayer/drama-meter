...
export class BuffState {
  ...
  groupedSynergies: Map<string, Map<number, StatusEffect>> = $derived.by(() => {
    ...
    for (const [id, buff] of Object.entries(this.enc.encounter.encounterDamageStats.buffs)) {
      ...
      filterStatusEffects(this.appContext.classesMap, this.appContext.supportClassIds, temp, buff, Number(id), this.focusedPlayer, this.tab);
    }
    for (const [id, debuff] of Object.entries(this.enc.encounter.encounterDamageStats.debuffs)) {
      ...
      filterStatusEffects(this.appContext.classesMap, this.appContext.supportClassIds, temp, debuff, Number(id), this.focusedPlayer, this.tab);
    }
  ...
  groupedShields: Map<string, Map<number, StatusEffect>> = $derived.by(() => {
    ...
    for (const [id, shield] of Object.entries(this.enc.encounter.encounterDamageStats.appliedShieldBuffs)) {
      filterStatusEffects(this.appContext.classesMap, this.appContext.supportClassIds, temp, shield, Number(id), undefined, undefined, true);
    }
  ...
  constructor(appContext: AppContext, enc: EncounterState) {
