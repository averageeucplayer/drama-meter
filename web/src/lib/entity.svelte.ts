...
import { EntityType, type AppContext, type Entity, type IncapacitatedEvent } from "./types";
...
export class EntityState {
  ...
  name: string = $derived.by(() => {
    ...
    if (this.entity.entityType === EntityType.ESTHER) {
      return getEstherFromNpcId(this.appContext.esthers, this.entity.npcId);
    } else {
  ...
  skills = $derived.by(() => {
    ...
    if (this.entity && this.entity.class === "Arcanist") {
      return Object.values(this.entity.skills)
        ...
        .filter((skill) => !this.appContext.cardIds.includes(skill.id));
    } else {
